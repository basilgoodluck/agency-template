...
export default function NotFound() {
  ...
  useEffect(() => {
    ...
    if (!canvas) return; // Null check to ensure canvas exists before using it
    ...
    // Pass the canvas to the Particle class constructor
    class Particle {
      x: number;
      y: number;
      size: number;
      ...
      update() {
        if (!canvas) return; // Null check to ensure canvas is available
        this.x += this.speedX;
        this.y += this.speedY;
    ...
    function init() {
      if (!canvas) return; // Null check to ensure canvas exists
      particles = [];
      for (let i = 0; i < particleCount; i++) {
        particles.push(new Particle(canvas)); // Pass canvas here
      }
    }

    function animate() {
      if (!canvas || !ctx) return; // Null checks
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      particles.forEach((particle) => {
        particle.update();
        particle.draw(ctx); // Pass the context here
      });
    ...
    const handleResize = () => {
      if (canvas) { // Ensure canvas exists before resizing
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        init();
      }
